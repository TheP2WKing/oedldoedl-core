...
public class ModFluidUtil {
	...
	public static FluidAttributes.Builder liquidFluidBuilder(int color) {
		return FluidAttributes.builder(LIQUID_STILL, LIQUID_FLOW).density(1000).viscosity(1000).luminosity(0)
				.temperature(300).color(color).sound(SoundEvents.ITEM_BUCKET_FILL, SoundEvents.ITEM_BUCKET_EMPTY);
	}

	public static FluidAttributes.Builder moltenFluidBuilder(int color) {
		return FluidAttributes.builder(MOLTEN_STILL, MOLTEN_FLOW).density(2000).viscosity(10000).luminosity(15)
				.temperature(1000).color(color)
